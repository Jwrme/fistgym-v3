...
import { FaChartLine, FaChartBar, FaChartPie, FaCalendarAlt, FaUsers, FaUserFriends, FaMoneyBillWave, FaTrophy, FaDownload, FaSync, FaHistory, FaTimes, FaEye } from 'react-icons/fa';
...
const FinancialAnalyticsDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // Fetch all users data (same endpoint as ALL MEMBERS page)
      const usersRes = await fetch('http://localhost:3001/api/users');
      const allUsers = await usersRes.json();
      
      // Calculate users count manually (exclude admin users like ALL MEMBERS page does)
      const totalUsers = allUsers.filter(user => !user.isAdmin).length;
      const usersCountData = { totalUsers, growth: 0 }; // For now, set growth to 0
      
      // Calculate dashboard metrics
      calculateDashboardMetrics(earningsData, membershipData, usersCountData);
  ...
  const calculateDashboardMetrics = (earningsData, membershipData, usersCountData = {}) => {
    const { startDate, endDate } = getDateRange(timeFilter);
    ...
    setDashboardData({
      ...
      userGrowth: usersCountData.growth || 0,
    });
  };
    ...
    <div style={{ padding: '20px', background: '#f8f9fa', minHeight: '100vh' }}>
      ...
      <div style={{ background: '#fff', borderRadius: 12, padding: 24, marginBottom: 20, boxShadow: '0 2px 12px rgba(0,0,0,0.08)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: 20, marginBottom: 20 }}>
          ...
          <div style={{ background: 'linear-gradient(135deg, #9b59b6, #8e44ad)', color: 'white', padding: 20, borderRadius: 12, boxShadow: '0 4px 12px rgba(155,89,182,0.3)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                <h3 style={{ margin: 0, fontSize: 14, opacity: 0.9 }}>Total Users</h3>
                <p style={{ margin: '8px 0 0 0', fontSize: 24, fontWeight: 'bold' }}>{dashboardData.totalUsers}</p>
                <p style={{ margin: '4px 0 0 0', fontSize: 12, opacity: 0.8 }}>↗ +{dashboardData.userGrowth}% vs last month</p>
              </div>
              <FaUserFriends style={{ fontSize: 32, opacity: 0.8 }} />
            </div>
          </div>
        </div>
      </div>
