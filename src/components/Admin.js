...
const Admin = () => {
  ...
  const handleAddRate = async () => {
    ...
    if (!newRateClassName.trim()) {
      setToast('Please enter a class name.');
      setTimeout(() => setToast(''), 2000);
      return;
    }
    
    if (!newRatePrice.trim() || isNaN(newRatePrice) || parseFloat(newRatePrice) <= 0) {
      setToast('Please enter a valid price.');
      setTimeout(() => setToast(''), 2000);
      return;
    }
    
    if (!newRateImageUrl.trim()) {
      setToast('Please enter an image URL.');
      setTimeout(() => setToast(''), 2000);
      return;
    }
    
    if (!newRateInfo.trim()) {
      setToast('Please enter rates information.');
      setTimeout(() => setToast(''), 2000);
      return;
    }
  ...
  const handleAddPromo = async () => {
    ...
    if (!newPromoName.trim()) {
      setPromoMessage('Please enter a promo name.');
      setTimeout(() => setPromoMessage(''), 2000);
      return;
    }
    
    if (!newPromoPrice.trim() || isNaN(newPromoPrice) || parseFloat(newPromoPrice) <= 0) {
      setPromoMessage('Please enter a valid price.');
      setTimeout(() => setPromoMessage(''), 2000);
      return;
    }
    
    if (!newPromoImageUrl.trim()) {
      setPromoMessage('Please enter an image URL.');
      setTimeout(() => setPromoMessage(''), 2000);
      return;
    }
    
    if (!newPromoInfo.trim()) {
      setPromoMessage('Please enter promo information.');
      setTimeout(() => setPromoMessage(''), 2000);
      return;
    }
              ...
              <div style={{ margin: '24px 0', overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', background: '#fff', borderRadius: 12, boxShadow: '0 2px 12px rgba(0,0,0,0.08)' }}>
                  ...
                  <tbody>
                    ...
                    {!usersLoading && !usersError && users.filter(user => !user.isAdmin).map(user => {
